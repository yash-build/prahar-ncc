...
const EditCadetForm = ({ cadet, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (cadet) {
      setForm({
        ...
        name: cadet.name || '',
        wing: cadet.wing || 'SD',
        gender: cadet.gender || 'M',
        yearOfStudy: cadet.yearOfStudy || 1,
        batchYear: cadet.batchYear || '2024-25',
        phone: cadet.phone || '',
        email: cadet.email || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await api.put(`/cadets/${cadet._id}`, formData, {
      ...
      toast.error(err.response?.data?.message || 'Failed to update cadet');
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div onClick={(e) => e.stopPropagation()} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-smoke rounded-sm w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-olive-dark p-4 flex justify-between items-center">
          <h2 className="text-khaki font-heading font-bold uppercase tracking-widest text-lg">Edit Cadet</h2>
          <button type="button" onClick={onClose} className="text-white/50 hover:text-white">✕</button>
        </div>
        <div className="p-6 overflow-y-auto">
          <form id="edit-cadet-form" onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2"><label className="label">Update Photo (Optional)</label>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
          ...
          <button form="edit-cadet-form" type="submit" className="btn-primary">Save Changes</button>
        </div>
      </motion.div>
    </div>
  );
